import { id } from './id';
import createCustomDataSource from './dataSource';
...
export default {
  ...
  getDataSourcesModule: ({ servicesManager, commandsManager, extensionManager }) => {
    //console.info('Calling getDataSourcesModule', servicesManager, extensionManager);
    return [
      {
        name: 'minimal-extension',
        type: 'local', // 'webApi' | 'local' | 'other'
        createDataSource: dataSourceConfig => {
          return createCustomDataSource(); //IWebApiDataSource.create(/* */);
        },
      },
    ];
  },
